...
class Test_TC208_012_SetServiceForAllClients:
	@pytest.fixture(autouse=True)
	def class_setup(self, one_time_setup):
		self.driver.maximize_window()
		self.logIn()

	def test_set_service_for_all_clients(self):
		common = CommonMethods(self.driver)
		self.log.info("starting test {}...".format(__name__))

		# get the required data
		client_portal_url = XLUtility.readData(self.path_1, 'client_portal_data', 24, 2)
		user_name = XLUtility.readData(self.path_1, 'client_portal_data', 24, 3)
		password = XLUtility.readData(self.path_1, 'client_portal_data', 24, 4)
		service_name = XLUtility.readData(self.path_1, 'client_portal_data', 24, 5)
		duration = XLUtility.readData(self.path_1, 'client_portal_data', 24, 6)
		service_type = XLUtility.readData(self.path_1, 'client_portal_data', 24, 7)
		therapist_grade = XLUtility.readData(self.path_1, 'client_portal_data', 24, 8)
		allowed_portal = XLUtility.readData(self.path_1, 'client_portal_data', 24, 9)
		video_bookable = XLUtility.readData(self.path_1, 'client_portal_data', 24, 10)
		service_fee = XLUtility.readData(self.path_1, 'client_portal_data', 24, 11)
		...
		if service_name in list_of_services:
			pass_counter = pass_counter + 1
		print("pass_counter=", pass_counter)
		if pass_counter == 2:
