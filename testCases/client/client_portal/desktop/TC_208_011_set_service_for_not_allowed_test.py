...
class Test_TC208_011_SetServiceForNotAllowed:
	@pytest.fixture(autouse=True)
	def class_setup(self, one_time_setup):
		self.driver.maximize_window()
		self.logIn()

	def test_set_service_for_not_allowed(self):
		common = CommonMethods(self.driver)
		self.log.info("starting test {}...".format(__name__))

		# get the required data
		client_portal_url = XLUtility.readData(self.path_1, 'client_portal_data', 22, 2)
		user_name = XLUtility.readData(self.path_1, 'client_portal_data', 22, 3)
		password = XLUtility.readData(self.path_1, 'client_portal_data', 22, 4)
		service_name = XLUtility.readData(self.path_1, 'client_portal_data', 22, 5)
		duration = XLUtility.readData(self.path_1, 'client_portal_data', 22, 6)
		service_type = XLUtility.readData(self.path_1, 'client_portal_data', 22, 7)
		therapist_grade = XLUtility.readData(self.path_1, 'client_portal_data', 22, 8)
		allowed_portal = XLUtility.readData(self.path_1, 'client_portal_data', 22, 9)
		video_bookable = XLUtility.readData(self.path_1, 'client_portal_data', 22, 10)
		service_fee = XLUtility.readData(self.path_1, 'client_portal_data', 22, 11)
