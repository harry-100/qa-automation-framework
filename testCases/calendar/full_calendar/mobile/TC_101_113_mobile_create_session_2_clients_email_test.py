...
# This test checks the functionality of creating a session with 2 linked clients with email
@pytest.mark.usefixtures("one_time_setup")
class Test_TC101_113_CreateSession2CLientsWithEmail():

	@pytest.fixture(autouse=True)
	def classSetup(self, one_time_setup):
		self.logIn()

	def test_create_session_2clients_withemail(self):
		common = CommonMethods(self.driver)
		self.log.info("starting test {}...".format(__name__))
		self.driver.set_window_size(411, 823)
		imap_obj = imapclient.IMAPClient('imap.gmail.com', ssl=True)
		email_id = XLUtility.readData(self.path, 'session_mobile_data', 26, 8)
		passwd = XLUtility.readData(self.path, 'session_mobile_data', 26, 9)
		email_subject = XLUtility.readData(self.path, 'session_mobile_data', 26, 7)
		...
		client_name = XLUtility.readData(self.path, 'session_mobile_data', 26, 3)
		sleep(1)
		...
		service = XLUtility.readData(self.path, 'session_mobile_data', 26, 5)
		sleep(2)
		...
		self.client_page_obj.clk_all_clients_mobile()
		sleep(1)
		client_name_2 = XLUtility.readData(self.path, 'session_mobile_data', 26, 6)
		...
		if exp_email_content in act_email_content:
			...
			self.driver.save_screenshot(
				self.pathScreenShot +
				"Test_TC101_113_CreateSession2CLientsWithEmail " + self.dateFormat + ".png"
			)
